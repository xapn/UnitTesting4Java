...
public class GlobalPositioningSystem {
    ...
    public boolean mustGoStraightOn() {
        ...
        if (!(roadMap.indicateBarrier(new Position(currentPosition, direction)))) {
            if (trajectory.isCollinearSameSense(direction)) {
                toutDroit = true;
                LOGGER.debug("Go straight on");
            } else if (trajectory.calculateScalarProduct(direction) > trajectory.calculateScalarProduct(direction
                    .getRightDirection())
                    && trajectory.calculateScalarProduct(direction) > trajectory.calculateScalarProduct(direction
                            .getLeftDirection())) {
                toutDroit = true;
                LOGGER.debug("Go straight on");
            }
    ...
    public boolean mustTurnAround() {
        ...
        if (((destination.getX() - currentPosition.getX()) * direction.getDirectionX() < 0)
                && ((destination.getY() - currentPosition.getY()) * direction.getDirectionY() < 0)) {
            around = true;
    ...
    public boolean mustTurnLeft() {
        ...
        Direction directionDestination = new Direction(destination.getX() - currentPosition.getX(), destination.getY()
                - currentPosition.getY());
        
        if (directionDestination.calculateScalarProduct(directionGauche) >= directionDestination
                .calculateScalarProduct(directionDroite)
                && !(roadMap.indicateBarrier(new Position(currentPosition, directionGauche)))) {
            onTheLeft = true;
    ...
    public boolean mustTurnRight() {
        ...
        if (directionDestination.calculateScalarProduct(directionDroite) >= directionDestination
                .calculateScalarProduct(directionGauche)
                && !(roadMap.indicateBarrier(new Position(currentPosition, directionDroite)))) {
            onTheRight = true;
    ...
    public void updateCurrentPosition() {
        ...
        LOGGER.debug(previous + " => " + currentPosition.toString() + " | " + direction.toString());
    }
}
